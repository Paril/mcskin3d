...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	public class FastZip
	{
		...
		public IEntryFactory EntryFactory
		{
			...
			set
			{
				if (value == null) entryFactory_ = new ZipEntryFactory();
				else entryFactory_ = value;
			}
		}
		                      ...
		                      string directoryFilter)
		{
			...
			{
#if !NETCF_1_0
				if (password_ != null) outputStream_.Password = password_;
#endif
		                       ...
		                       bool isStreamOwner)
		{
			...
			{
#if !NETCF_1_0
				if (password_ != null) zipFile_.Password = password_;
#endif
		...
		private void ProcessFile(object sender, ScanEventArgs e)
		{
			...
			if (e.ContinueRunning)
			{
				...
				{
					if (events_ != null) continueRunning_ = events_.OnFileFailure(e.Name, ex);
					else
					{
		...
		private void ExtractFileEntry(ZipEntry entry, string targetName)
		{
			...
			if (overwrite_ != Overwrite.Always)
			{
				...
				{
					if ((overwrite_ == Overwrite.Prompt) && (confirmDelegate_ != null)) proceed = confirmDelegate_(targetName);
					else proceed = false;
				}
			}
			...
			if (proceed)
			{
				...
				if (continueRunning_)
				{
					try
					{
						...
						if (restoreDateTimeOnExtract_) File.SetLastWriteTime(targetName, entry.DateTime);
					...
					{
						if (events_ != null) continueRunning_ = events_.OnFileFailure(targetName, ex);
						else
						{
							continueRunning_ = false;
							throw;
		...
		private void ExtractEntry(ZipEntry entry)
		{
			...
			if (doExtraction)
			{
				if (entry.IsFile) targetName = extractNameTransform_.TransformFile(targetName);
			...
			if (doExtraction && !Directory.Exists(dirName))
			{
				if (!entry.IsDirectory || CreateEmptyDirectories)
				{
					...
					catch (Exception ex)
					{
						...
						if (events_ != null)
						{
							if (entry.IsDirectory) continueRunning_ = events_.OnDirectoryFailure(targetName, ex);
							else continueRunning_ = events_.OnFileFailure(targetName, ex);
						}
						else
						{
							continueRunning_ = false;
