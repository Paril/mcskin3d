...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	public sealed class ZipExtraData : IDisposable
	{
		...
		public int UnreadCount
		{
			get
			{
				if ((_readValueStart > _data.Length) ||
				    (_readValueStart < 4)) throw new ZipException("Find must be called before calling a Read method");
		...
		{
			Stream result = null;
			if (Find(tag)) result = new MemoryStream(_data, _index, _readValueLength, false);
			return result;
		}
		...
		public bool Find(int headerID)
		{
			...
			while ((localTag != headerID) && (_index < _data.Length - 3))
			{
				...
				if (localTag != headerID) _index += localLength;
			}
		...
		{
			if (_index > _data.Length - 2) throw new ZipException("End of extra data");
