	...
	public class ZipEntry : ICloneable
	{
		...
		public int ExternalFileAttributes
		{
			get
			{
				...
				else return externalFileAttributes;
			}
		...
		private bool HasDosAttributes(int attributes)
		{
			...
			if ((known & Known.ExternalAttributes) != 0)
			{
				if (((HostSystem == (int) HostSystemID.Msdos) ||
				     (HostSystem == (int) HostSystemID.WindowsNT)) &&
				    (ExternalFileAttributes & attributes) == attributes) result = true;
			}
			return result;
		}
		...
		public int Version
		{
			get
			{
				...
				if (versionToExtract != 0) return versionToExtract;
				else
				{
					int result = 10;
					if (AESKeySize > 0) result = ZipConstants.VERSION_AES; // Ver 5.1 = AES
					else if (CentralHeaderRequiresZip64) result = ZipConstants.VersionZip64;
...
//				return (byte[]) extra.Clone();
		...
		{
			get { return (byte) _aesEncryptionStrength; }
		}
#else
	/// <summary>
