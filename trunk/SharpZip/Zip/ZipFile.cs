...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		{
			get { return (ZipEntry) entries_[index].Clone(); }
		}
		...
		public int FindEntry(string name, bool ignoreCase)
		{
			...
			for (int i = 0; i < entries_.Length; i++)
				if (string.Compare(name, entries_[i].Name, ignoreCase, CultureInfo.InvariantCulture) == 0) return i;
			return -1;
		}
		...
		private long TestLocalHeader(ZipEntry entry, HeaderTest tests)
		{
			lock (baseStream_)
			{
				...
				if (testHeader)
				{
					...
					if ((localFlags &
					     (int) (GeneralBitFlags.ReservedPKware4 | GeneralBitFlags.ReservedPkware14 | GeneralBitFlags.ReservedPkware15)) !=
					    0)
					...
					{
						throw new ZipException(string.Format("Version required to extract this entry is too low for encryption ({0})",
						                                     extractVersion));
					}
		...
		public IEntryFactory EntryFactory
		{
			...
			set
			{
				if (value == null) updateEntryFactory_ = new ZipEntryFactory();
				else updateEntryFactory_ = value;
			}
		}
			...
			try
			{
				...
				else
				{
					...
					if (entries_.Length == 0)
					{
						...
						using (var zhs = new ZipHelperStream(baseStream_)) zhs.WriteEndOfCentralDirectory(0, 0, 0, theComment);
					}
				}
			}
			finally
 ...
 */
		...
		private void UpdateCommentOnly()
		{
			...
			if (archiveStorage_.UpdateMode == FileUpdateMode.Safe) Reopen(archiveStorage_.ConvertTemporaryToFinal());
			else ReadEntries();
		}
		...
		private class ZipUpdate
		{
			...
			*/
			...
			public ZipEntry OutEntry
			{
				get
				{
					if (outEntry_ == null) outEntry_ = (ZipEntry) entry_.Clone();
		...
		private class ZipString
		{
			...
			public void Reset()
			{
				if (isSourceString_) rawComment_ = null;
				else comment_ = null;
			}
