	...
	public class WindowsNameTransform : INameTransform
	{
		...
		public char Replacement
		{
			...
			set
			{
				...
				if ((value == '\\') || (value == '/')) throw new ArgumentException("invalid replacement character");
		...
		public string TransformDirectory(string name)
		{
			...
			if (name.Length > 0) while (name.EndsWith(@"\")) name = name.Remove(name.Length - 1, 1);
		...
		public string TransformFile(string name)
		{
			if (name != null)
			{
				...
				if (_trimIncomingPaths) name = Path.GetFileName(name);
